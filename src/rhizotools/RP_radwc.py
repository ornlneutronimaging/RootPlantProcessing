...
def RP_radwc(parameters):
    ...
    data_num = np.zeros(
        [int(np.shape(radrange)[0]) - 1, int(np.shape(range(0, int(np.floor(maxdistval)), pixelbin))[0])]
    )
    ...
    for i in range(0, np.shape(newdistrange)[0] - 1):
        for j in range(1, np.shape(radrange)[0]):
            ...
            image_pos = (
                (image_rootdist >= newdistrange[i])
                & (image_rootdist < newdistrange[i + 1])
                & (image_distmap == radrange[j])
            )
