...
def RP_run(wd, wd_userconfig="", analysis_list=[], parameters_=0, override=0):  # noqa: C901
    """
    SUMMARY:
    ...
    if type(analysis_list) is not list:
        raise ValueError(
            "Analysis list must be in 'list' format, not as a string.  Please put [] around the analysis terms."
        )
    ...
    if type(analysis_list) is list:
        ...
        if counter == 0:
            raise ValueError(
                "Analysis list does not have a valid processing choice.  Please enter a correct processing choice."
            )
        ...
        if not IOcheck == 10:
            if IOcheck == 0:
                raise ValueError(
                    ...
                    "Please ensure that either you have your input files,"
                    "or the user_config.txt file specified the correct"
