...
def RP_userconfiganalysis(wd, analysis_pos, analysis):  # noqa: C901
    ...
    for line in f:
        if counter == analysis_pos[0] - 1:
            ...
            if errorflag == 1:
                raise ValueError(
                    "user_config.txt is in an incorrect format."
                    "Check if additional lines or text were added or modified in the file."
                )
        if (counter > analysis_pos[0] - 1) & (counter < analysis_pos[1] + 1):
            # Skip blank lines or lines without a colon
            if ":" in line:
    ...
    if analysis == "RP_stitch":
        try:
            parameters["dimh_horzoffset"] = int(parameters["dimh_horzoffset"])
            parameters["dimh_vertoffset"] = int(parameters["dimh_vertoffset"])
            parameters["dimv_horzoffset"] = int(parameters["dimv_horzoffset"])
            parameters["dimv_vertoffset"] = int(parameters["dimv_vertoffset"])
        except ValueError:
    ...
    if analysis == "RP_wc":
        try:
            parameters["b_w"] = float(parameters["b_w"])
            parameters["s_w"] = float(parameters["s_w"])
            parameters["s_a"] = float(parameters["s_a"])
            parameters["s_s"] = float(parameters["s_s"])
            parameters["x_s"] = float(parameters["x_s"])
            parameters["x_a"] = float(parameters["x_a"])
        except ValueError:
    ...
    if analysis == "RP_mask":
        try:
            parameters["windowsize"] = int(parameters["windowsize"])
            parameters["threshold"] = float(parameters["threshold"])
            parameters["globthresh"] = float(parameters["globthresh"])
        except ValueError:
    ...
    if analysis == "RP_imagefilter":
        try:
            parameters["bwareaval"] = int(parameters["bwareaval"])
            parameters["medfilterval"] = int(parameters["medfilterval"])
        except ValueError:
    ...
    if analysis == "RP_distmap":
        try:
            parameters["maxval"] = int(parameters["maxval"])
        except ValueError:
    ...
    if analysis == "RP_radwc":
        try:
            parameters["pixelbin"] = int(parameters["pixelbin"])
        except ValueError:
